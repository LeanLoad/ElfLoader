...
theorem segment_mmapRange_in_rsv (bp : BoundPlan) (i : Fin bp.objCount)
    (j : Fin (bp.elfAt i).segments.size) :
    Runtime.InRange (bp.baseAt i + (bp.segAt i j).pageEaddr)
...
--
-- Both end at the same shape: `Disjoint (base + sp.pageEaddr)
...
theorem segment_storeRange_in_rsv (bp : BoundPlan) (i : Fin bp.objCount)
    ...
    (h_cov : Parse.coversRela
      (bp.segAt i j).segment.eaddr (bp.segAt i j).segment.memsz r_offset)
