...
def elfIdent? : Option ElfIdent :=
  Decoder.run? elfIdentBytes (Decodable.decoder (α := ElfIdent))
...
#guard (Decoder.run? badMagicElfIdentBytes (Decodable.decoder (α := ElfIdent))).isNone
...
#guard (Decoder.run? badClassElfIdentBytes (Decodable.decoder (α := ElfIdent))).isNone

def truncatedElfIdentBytes : ByteArray :=
  elfIdentBytes.extract 0 15

#guard (Decoder.run? truncatedElfIdentBytes (Decodable.decoder (α := ElfIdent))).isNone
