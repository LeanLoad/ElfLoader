...
#guard dynBytes.size == 14 * Dyntab.EntrySize  -- = 224
...
-- ── Error cases ──────────────────────────────────────────────────────
-- Truncated entry: 10 bytes when 16 (Dyntab.EntrySize) expected — EOF inside
-- the `d_un` u64 read.
#guard
  (decodeBytes? (α := Dyntab.Entry) (dynBytes.extract 0 10)).isNone
