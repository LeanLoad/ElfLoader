/-
Read dynamic ELF content through the checked file view.

This module follows `.dynamic` table pointers through `FileView`, reads the
pointed-to file bytes with `FileReader`, and builds the transient `Dynamic`
staging image.
-/
...
private def decodeAtRange [Monad m] (r : FileReader m) (view : FileView)
    (range : EaddrRange) (decoder : Decoder α) : ExceptT String m α :=
  match FileView.mapRange view r.fileSize range with
  | .ok mapped => decodeAtFileRange r mapped.fileRange decoder
  | .error e   => throw e

/-- Read `DT_HASH`'s SysV `nchain`, the dynamic-symbol count. -/
private def readSymCount [Monad m] (r : FileReader m) (view : FileView)
    (hashVa : Option Eaddr) : ExceptT String m Nat := do
  match hashVa with
  | none    => pure 0
  | some va =>
      let range : EaddrRange := { start := va, size := RawSysVHash.byteSize }
      let hdr ← decodeAtRange r view range RawSysVHash.parse
      pure hdr.symCount

/-- Read `.dynstr` bytes. UTF-8 validation is delayed until `StrtabEntry`. -/
private def readStrtab [Monad m] (r : FileReader m) (view : FileView)
    (loc : Option EaddrRange) : ExceptT String m Strtab :=
  match loc with
  | none       => pure Strtab.empty
  | some range => decodeAtRange r view range Strtab.parse

/-- Read `.dynsym`; count comes from `DT_HASH.nchain`. -/
private def readSymtab [Monad m] (r : FileReader m) (view : FileView)
  ...
  | some va =>
      ...
      decodeAtRange r view range (RawSymtab.parse count)
...
/-- Read `.dynsym` and its SysV hash header. LeanLoad requires `DT_SYMTAB` and
    ...
    count. -/
private def readSymtabData [Monad m] (r : FileReader m) (view : FileView)
...
private def readRelas [Monad m] (label : String) (r : FileReader m)
    (view : FileView) (loc : Option EaddrRange) :
  ...
  | some range =>
      ...
      decodeAtRange r view range (RawRela.parseTable count)

/-- Read `DT_JMPREL` after validating its separate `DT_PLTREL` encoding tag. -/
private def readJmprel [Monad m] (r : FileReader m) (view : FileView)
...
private def readEaddrArray [Monad m] (label : String) (r : FileReader m)
  ...
  | some range =>
      ...
      if bytes % 8 == 0 then
        decodeAtRange r view range (decodeArray (α := Eaddr) (bytes / 8))
      else
...
private def fetchDynamicData [Monad m] (r : FileReader m)
    (view : FileView) (dyn : Dyntab) : ExceptT String m DynamicData := do
...
def readM [Monad m] (r : FileReader m) : ExceptT String m Dynamic := do
  ...
  let view ←
    match FileView.ofHeaders r.fileSize header programHeaders with
    | .ok view => pure view
    | .error e => throw e
  let dyn ← match programHeaders.find? (·.p_type == .dynamic) with
    | none    => pure #[]
    | some ph => decodeAt r ph.p_offset ph.p_filesz
