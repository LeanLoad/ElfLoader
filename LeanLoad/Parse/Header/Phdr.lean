...
`p_type` is decoded into a range-aware `PhdrType`: the named gABI
values are constructors, while OS-specific, processor-specific, and
future-reserved values preserve their raw tag.
-/
...
instance : ByteMap PhdrType UInt32 where
  ofRaw
  | 0 => .ok .null
  | 1 => .ok .load
  | 2 => .ok .dynamic
...
def PT_LOAD    : PhdrType := .load
def PT_DYNAMIC : PhdrType := .dynamic
...
theorem vaToOffset_eq_some
  ...
  · rename_i hcond
    ...
    refine ⟨ph, h_mem, h_load, h_lo, h_hi, ?_⟩
