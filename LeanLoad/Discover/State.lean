/-
DFS construction carrier.

`DfsState` holds the in-progress `LoadGraph` (objects + deps + a
name→idx HashMap accelerator) plus the invariants we maintain as
we recurse. Its field naming intentionally mirrors `LoadGraph` so
the final promotion (`Driver.discoverWith`'s `LoadGraph` mk) is
mostly structural.
...
plus the `rowSum` / `postOrder*` invariants that `Driver` consumes.
The fifth `LoadGraph` invariant — `closure` — is *not* carried
here; individual push/recordDep steps break it. `closure` is
established at the very end of `Driver.discoverWith`, when every
...
Plus characterisation theorems (`*_size`, `*_postOrder`,
`*_pending_*`) used by `Driver.dfs` / `dfsList` to discharge the
`DfsResult` / `NeededAcc` invariants they thread through the
recursion.
-/
...
-- ============================================================================
-- Characterisation theorems — used by `Driver.dfs` / `dfsList` to
-- discharge the per-iteration `DfsResult` / `NeededAcc` invariants.
-- ============================================================================
