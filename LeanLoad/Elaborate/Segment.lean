...
Carried as struct fields:
  ...
  - LeanLoad's 48-bit address-space bound (`addrBound`), which lets
    page-arithmetic proofs in `Layout.Region` ignore UInt64 wrap,
...
BSS bounds, POSIX `PROT_*`) live on `Layout.Region`, which couples a
...
/-- Typed segment permission — generic 3-bit `read/write/execute`
    ...
    that `mprotect` consumes (translation lives in `Layout.Region`). -/
...
structure Segment where
  ...
  /-- 48-bit address-space bound. **Not gabi.** LeanLoad assumes
      ...
      alignment; the bound is what lets `Layout.Region`'s
