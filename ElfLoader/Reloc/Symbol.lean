...
BFS walk over `LoadGraph` lives in `ResolutionOrder.lean`; the across-elves
wrapper lives in `Resolve.lean`.
-/

import ElfLoader.Parse
import ElfLoader.Parse.Symbol.Checked

namespace ElfLoader.Reloc

open ElfLoader
open ElfLoader.Parse (Elf Symbol)
