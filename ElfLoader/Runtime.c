/* ElfLoader runtime shims — single trust seam between Lean and the
 * kernel. Counterpart to `ElfLoader/Runtime/IO.lean`.
 *
 * Two topic groups:
 ...
 *   2. Control transfer — `elfloader_exec_run` builds the
...
static inline lean_object * elfloader_io_err(const char * msg) {
 ...
 * not open. */
LEAN_EXPORT lean_object * elfloader_open_path(
...
LEAN_EXPORT lean_object * elfloader_canonical_origin_dir(
        b_lean_obj_arg path_obj,
        lean_object * /* w */) {
    char resolved[PATH_MAX];
    const char * path = lean_string_cstr(path_obj);
    if (realpath(path, resolved) == NULL) {
        return elfloader_io_err(strerror(errno));
    }
...
LEAN_EXPORT lean_object * elfloader_file_size(uint32_t fd, lean_object * /* w */) {
    struct stat st;
    if (fstat((int)fd, &st) != 0) {
        return elfloader_io_err(strerror(errno));
    }
    if (!S_ISREG(st.st_mode) || st.st_size < 0) {
        return elfloader_io_err("open file is not a regular file with a nonnegative size");
    }
...
LEAN_EXPORT lean_object * elfloader_pread(uint32_t fd,
                                         uint64_t offset,
                                         uint64_t len,
                                         lean_object * /* w */) {
    ...
    if (n != (ssize_t)len) {
        lean_dec_ref(arr);
        return elfloader_io_err(n < 0 ? strerror(errno) : "short read");
    }
...
LEAN_EXPORT lean_object * elfloader_mmap_file(uint32_t fd,
                                             uint64_t vaddr,
                                             uint64_t len,
                                             uint32_t prot,
                                             uint64_t offset,
...
LEAN_EXPORT lean_object * elfloader_mmap_anon(uint64_t len,
                                             lean_object * /* w */) {
...
LEAN_EXPORT lean_object * elfloader_mprotect(uint64_t addr,
                                            uint64_t len,
                                            uint32_t prot,
                                            lean_object * /* w */) {
...
LEAN_EXPORT lean_object * elfloader_store(uint64_t addr,
                                         uint8_t size,
                                         uint64_t value,
                                         lean_object * /* w */) {
...
LEAN_EXPORT lean_object * elfloader_zero(uint64_t addr,
                                        uint64_t len,
                                        lean_object * /* w */) {
...
LEAN_EXPORT lean_object * elfloader_exec_call_ctor(uint64_t addr,
                                                  lean_object * /* w */) {
    if (addr == 0) return elfloader_io_err("call_ctor: null address");
    typedef void (*ctor_t)(void);
    ((ctor_t)(uintptr_t)addr)();
    return lean_io_result_mk_ok(lean_box(0));
}

LEAN_EXPORT lean_object * elfloader_exec_run(
    uint64_t entry,
    uint64_t phdr_va,
    uint64_t phent,
    uint64_t phnum,
    ...
    if (entry == 0) return elfloader_io_err("exec: null entry");
    if (!stack_va || stack_len < 4096) {
        return elfloader_io_err("exec: stack region missing or too small");
    }
    ...
    *w++ = 0;                   /* envp[0] = NULL        */
     ...
     * deadlocks against libuv's pthread lock when elfloader's own
    ...
    asm volatile(
        ...
        : "memory");
#else
# error "elfloader_exec_run: unsupported architecture (need __aarch64__ or __x86_64__)"
#endif
