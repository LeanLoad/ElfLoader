...
Phase 2 of 2 in the relocation pipeline:
  ...
  1. **Layout** (`Layout/Reloc.lean`) — `RawRela → Entry objCount seg`. Pure,
...
open ElfLoader.Parse (Segment)
open ElfLoader.Parse (Elf)
open ElfLoader.Reloc.ABI (Formula FormulaInputs FormulaResult PatchSize)
...
private def symValueOf (elfs : Array Elf) (bases : Array UInt64)
    ...
    (target : Reloc.Target elfs.size) : UInt64 :=
...
-- `Reloc.covered` witness on the entry comes via `entry.covered`.
-- ============================================================================
