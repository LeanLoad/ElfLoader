...
instance : DecodableFromScalar ProgramHeaderType UInt32 where
  fromScalar
  | 0 => .ok .null
  | 1 => .ok .load
  | 2 => .ok .dynamic
...
instance : DecodableFromScalar ProgramHeaderFlags UInt32 where
  fromScalar flags := .ok (ProgramHeaderFlags.ofRaw flags)
