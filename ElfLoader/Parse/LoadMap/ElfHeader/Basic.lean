...
import ElfLoader.Parse.LoadMap.ElfHeader.Raw
...
/-- ELF header accepted by ElfLoader's load-map stage. Field layout matches
    `Elf64_Ehdr`, with ElfLoader's header policy attached as proof fields.
...
structure ElfHeader (fileSize : ByteSize) extends RawElfHeader where
  /-- ElfLoader supports only ELFCLASS64 inputs. -/
  class64     : ident.ei_class = .class64
  /-- ElfLoader targets little-endian psABIs. -/
  ...
  /-- ElfLoader supports PIE/shared-object style `ET_DYN`, not fixed-address `ET_EXEC`. -/
  notExec     : e_type ≠ .exec
