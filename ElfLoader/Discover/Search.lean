...
/-- Default directories after `RPATH`/`LD_LIBRARY_PATH`/`RUNPATH` fail.
    ...
    gABI/psABI, so ElfLoader records this deterministic Linux-oriented policy here. -/
...
private def substitutionReplacement (originDir : Option String) (name : String) :
  ...
  if name == "ORIGIN" then
    ...
    -- other name is unspecified, so ElfLoader rejects it instead of guessing.
...
/-- Candidate paths for one dependency search, before exact `open(2)`.
    ...
    * Privileged/setuid restrictions are not modeled: ElfLoader does not perform
...
@[extern "elfloader_canonical_origin_dir"]
